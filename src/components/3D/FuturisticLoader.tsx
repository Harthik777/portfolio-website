...
export function FuturisticLoader() {
  ...
  useEffect(() => {
    ...
    const steps = isMobile ? (isLowEndDevice ? 30 : 40) : 60;
  ...
  useEffect(() => {
    ...
    const animate = (timestamp: number) => {
      ...
      const shouldSkipFrame =
        (isLowEndDevice && frameCount % 4 !== 0) ||
        (isMobile && !isLowEndDevice && frameCount % 3 !== 0);
      ...
      if (!shouldSkipFrame) {
        ...
        nodes.forEach((node, i) => {
          ...
          // Draw connections
          if (!simpleMode || i % 2 === 0) {
            // Draw fewer connections in simple mode
            node.connections.forEach(connectedIndex => {
              ...
              if (distance < maxDistance) {
                ...
                const pulseOpacity = isLowEndDevice
                  ? opacity * 0.5
                  : opacity *
                    (0.3 + 0.7 * Math.sin(animationTime * 2 + i * 0.1));
                ...
                if (isLowEndDevice) {
                  ...
                  if (resolvedTheme === 'dark') {
                    ...
                    if (!isMobile) {
                      gradient.addColorStop(
                        0.5,
                        `rgba(236, 72, 153, ${pulseOpacity * 0.8})`
                      );
                    }
                  } else {
                    gradient.addColorStop(
                    ...
                    gradient.addColorStop(
                      1,
                      `rgba(37, 99, 235, ${pulseOpacity})`
                    );
                    // Only add middle color stop on higher-end devices
                    if (!isMobile) {
                      gradient.addColorStop(
                        0.5,
                        `rgba(219, 39, 119, ${pulseOpacity * 0.8})`
                      );
                    }
                  }
          ...
          if (isLowEndDevice) {
            ...
            if (resolvedTheme === 'dark') {
              ...
              nodeGradient.addColorStop(
                1,
                `rgba(139, 92, 246, ${nodeOpacity * 0.5})`
              );
            } else {
              nodeGradient.addColorStop(0, `rgba(79, 70, 229, ${nodeOpacity})`);
              nodeGradient.addColorStop(
                1,
                `rgba(79, 70, 229, ${nodeOpacity * 0.3})`
              );
            }
          ...
          <div className="relative z-10 text-center">
              ...
              <div className="relative mx-auto h-20 w-20">
                ...
                {isLowEndDevice ? (
                  <div
                    ...
                    style={{
                      borderTopColor:
                        resolvedTheme === 'dark' ? '#8b5cf6' : '#4f46e5',
                      borderRightColor:
                        resolvedTheme === 'dark' ? '#ec4899' : '#db2777',
            ...
            >
              <div className="mb-2 flex justify-between text-sm font-medium">
                <span
                  style={{
                    color: resolvedTheme === 'dark' ? '#a1a1aa' : '#64748b',
                  }}
                >
                  Loading...
                </span>
