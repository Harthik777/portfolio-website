...
const deviceProfiles = [
  {
    ...
    mobile: true,
    userAgent:
      'Mozilla/5.0 (Linux; Android 4.4.2; Nexus 4 Build/KOT49H) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.77 Mobile Safari/537.36',
  },
  {
...
deviceProfiles.forEach((profile, index) => {
  console.log(
    `${index + 1}. ${profile.name} (${profile.width}x${profile.height})`
  );
});
...
function runTestForProfile(profile) {
  ...
  console.log(
    'Please check the browser window that opens and assess performance.'
  );
...
function askForNextTest() {
  rl.question(
    '\nEnter the number of the device profile to test (or 8 to exit): ',
    answer => {
